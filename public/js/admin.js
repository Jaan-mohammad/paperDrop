...
async function loadStats() {
  try {
    ...
    document.getElementById("statTotal").textContent = allData.notes.length;
    document.getElementById("statUsers").textContent = usersData.users.length;
    document.getElementById("statApproved").textContent = allData.notes.filter(
      (n) => n.status === "approved",
    ).length;
  } catch (error) {
        ...
        <div class="row align-items-center">
          <div class="col-12 col-md-8">
            ...
            <p style="font-size:0.8rem;
              ...
              ${u.email} •
              <span style="color:${
                u.role === "admin" ? "var(--accent)" : "var(--text-muted)"
              };">
                ${u.role}
              </span>
            </p>
